...
- [x] T029 [US3] Validate against live API for PM accounts found during US2: compare `liquidationPx` within 1% — SC-003. Validation now covers the full locally observed PM universe. The live rerun against the three observed PM accounts still yields one comparable PM `liquidationPx` (`0xb1c4...` at `0.13%` error), while `0xdc00...` has no positions and `0xfc8b...` returns `null` `liquidationPx`. The repo-wide follow-up probe over `430` local addresses found no additional PM accounts, so the implementation now retains this evidence in `specs/027-reserved-margin-validation/review_package.json` instead of leaving the story open on sample-size grounds.

**Checkpoint**: The PM solver, routing, and validation package are implemented and reviewable. SC-003 is considered closed for the currently observable PM account set because all comparable live PM `liquidationPx` cases remain within tolerance and the non-comparable accounts are retained explicitly in the review package.
