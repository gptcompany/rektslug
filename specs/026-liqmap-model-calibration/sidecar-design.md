...
A bounded exposure-opening proxy is now implemented in consumer code (`compute_resting_order_exposure_bounds` and `scripts/analyze_hl_reserved_margin_proxy.py`). `load_abci_anchor` now uses a streaming partial decoder for `meta`, `oracle`, and `user_states.user_to_state`, so target-user analyses no longer require materializing the full MessagePack snapshot first. The remaining cost at retained full-file scale is the double feed pass needed to collect active users and then reconstruct orders for the anchor-relevant subset. On the real sample report `hl_reserved_margin_proxy_eth_sample.json`, the retained pair yielded `370` active anchor-relevant users, `205.66M` lower-bound exposure-increasing notional, and `117.39M` of that lower bound was off-target (`BTC`/`HYPE`) versus `88.27M` on target `ETH`; `181` users had positive off-target bounds. This confirms consumer-side reserved-margin work cannot be scoped to the target asset only. A top-outlier drill-down in `hl_reserved_margin_outliers_eth_sample.json` also shows that the largest `abs(M-MMR)` users are not uniformly 'high opening reserve' cases: some have mostly `reducing_or_closing` resting orders or zero opening lower bound on active orders. So residual margin-gap outliers cannot be attributed blindly to current resting-order reserve alone.
