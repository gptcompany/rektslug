...
- [ ] T004 Introduce an explicit `rektslug-ank` profile/config surface in the local workflow
- [ ] T005 Ensure manifests/reports record the active local profile name
- [ ] T006 Add tests that distinguish `rektslug-default` from `rektslug-ank`
...
- [ ] T013 Re-run the full `BTC/ETH x 1d/1w` matrix against CoinAnk
- [ ] T014 Confirm the calibrated profile improves the majority of target metrics
