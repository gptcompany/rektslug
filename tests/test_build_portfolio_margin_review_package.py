...
def test_build_review_package_summarizes_fixture_accounts(tmp_path):
    ...
    assert "coverage is explicitly checked" in payload["residual_note"]


def test_build_review_package_fails_on_fixture_coverage_mismatch(tmp_path):
    fixture_path = Path("tests/fixtures/hyperliquid/portfolio_margin_live_cases.json")
    accounts_payload = {
        "portfolio_margin_accounts": [
            {"user": "0xb1c4a17f0f39c2b04333831104a82e94ab808510"},
    ...
    repo_scan_payload = {
        "pm_candidates": [
            {"user": "0xb1c4a17f0f39c2b04333831104a82e94ab808510", "mode": "portfolio_margin"},
            {"user": "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "mode": "portfolio_margin"},
        ]
    }
    ...
    try:
        build_review_package(fixture_path, accounts_path, repo_scan_path)
    except ValueError as exc:
        assert "coverage mismatch" in str(exc)
    else:
        raise AssertionError("expected coverage mismatch to fail")
