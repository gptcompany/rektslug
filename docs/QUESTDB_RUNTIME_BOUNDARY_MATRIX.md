...
| `GET /liquidations/heatmap-timeseries` hot window | QuestDB for live snapshots | QuestDB live reconstruction for recent windows | In-memory cache only | Falls back to DuckDB live compute when QuestDB is unavailable; skips DuckDB precomputed cache on purpose | **QuestDB-hot with DuckDB fallback** | Keep reducing fallback surface only if QuestDB live becomes sufficient under failure |
