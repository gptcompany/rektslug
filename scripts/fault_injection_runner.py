...
if fault_point == "PRE_SUBMIT":
    ...
    original_on_event = LiquidationRedisSignalStrategy.on_event
    def mocked_on_event(self, event):
        if event.__class__.__name__ == "OrderFilled":
            print("INJECTED FAULT: POST_SUBMIT_PRE_FILL")
            # Force a hard exit to simulate crash after submit but before fill is processed
            os._exit(1)
        original_on_event(self, event)
    LiquidationRedisSignalStrategy.on_event = mocked_on_event
...
"""
    ...
    if "INJECTED FAULT" in result.stdout or args.fault in ["REDIS_UNAVAILABLE", "DUCKDB_UNAVAILABLE"]:
    ...
    # Needs a real redis host for recovery even if REDIS_UNAVAILABLE was tested
    recovery_result = subprocess.run(recovery_cmd, env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    logger.info(f"Recovery run exit code: {recovery_result.returncode}")
    
    if "NODE_SMOKE_OK" in recovery_result.stdout:
        logger.info("Recovery run PASSED (NODE_SMOKE_OK). Account is flat.")
    else:
        logger.error("Recovery run FAILED.")
        logger.error(f"Stdout:\n{recovery_result.stdout}")

    # Output simple marker for the test script
    print(f"FAULT_TEST_RESULT: {args.fault} -> RECOVERY_CODE={recovery_result.returncode}")
